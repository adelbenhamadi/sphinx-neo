...
void DoSearch ( CSphIndex * pIndex )
{
	...
	tQuery.m_sQuery = "@comment_text italy";
...
CSphSource_MySQL * SpawnSource ( const char * sQuery, ISphTokenizer * pTok, CSphDict * pDict )
{
	...
	CSphColumnInfo tCol;
	tCol.m_eAttrType = ESphAttr::SPH_ATTR_INTEGER;
	tCol.m_sName = "story_id";
	tParams.m_dAttrs.Add ( tCol );
	tCol.m_eAttrType = ESphAttr::SPH_ATTR_TIMESTAMP;
	tCol.m_sName = "story_time";
	tParams.m_dAttrs.Add ( tCol );
...
int main ( int argc, char ** argv )
{
	...
	CSphDict * pDict = sphCreateDictionaryKeywords ( tDictSettings, NULL, pTok, "rt1", sError );
	CSphSource_MySQL * pSrc = SpawnSource ( "SELECT comment_id,comment_text,comment_author,story_id,story_time "
		"FROM hacker_news WHERE comment_id % 2=0 limit 0,10000", pTok, pDict );

	ISphTokenizer * pTok2 = sphCreateUTF8Tokenizer();
	CSphDict * pDict2 = sphCreateDictionaryKeywords ( tDictSettings, NULL, pTok, "rt2", sError );
	CSphSource_MySQL * pSrc2 = SpawnSource ( "SELECT comment_id,comment_text,comment_author,story_id,story_time "
		"FROM hacker_news WHERE comment_id % 2=1 limit 0,10000", pTok2, pDict2 );
