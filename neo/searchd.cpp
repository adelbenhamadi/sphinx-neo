...
int SearchRequestBuilder_t::CalcQueryLen ( const char * sIndexes, const CSphQuery & q, bool bAgentWeight ) const
{
	...
	ARRAY_FOREACH ( i, q.m_dOverrides )
		iReqSize += 12 + q.m_dOverrides[i].m_sAttr.Length() + // string attr-name; int type; int values-count
			( q.m_dOverrides[i].m_eAttrType==ESphAttr::SPH_ATTR_BIGINT ? 16 : 12 )*q.m_dOverrides[i].m_dValues.GetLength(); // ( bigint id; int/float/bigint value )[] values
	if ( q.m_bHasOuter )
...
void SearchRequestBuilder_t::SendQuery ( const char * sIndexes, NetOutputBuffer_c & tOut, const CSphQuery & q, bool bAgentWeight, int iWeight ) const
{
	...
	ARRAY_FOREACH ( i, q.m_dOverrides )
	{
		...
		ARRAY_FOREACH ( j, tEntry.m_dValues )
		{
			...
			{
				case ESphAttr::SPH_ATTR_FLOAT:	tOut.SendFloat ( tEntry.m_dValues[j].m_fValue ); break;
				case ESphAttr::SPH_ATTR_BIGINT:	tOut.SendUint64 ( tEntry.m_dValues[j].m_uValue ); break;
		...
		if ( iMatches )
		{
			...
			ARRAY_FOREACH ( i, tRes.m_dMatches )
			{
				...
				for ( int j=0; j<tSchema.GetAttrsCount(); j++ )
				{
					...
					} else if ( tAttr.m_eAttrType==ESphAttr::SPH_ATTR_BIGINT )
					{
					...
					} else if ( tAttr.m_eAttrType==ESphAttr::SPH_ATTR_STRINGPTR )
					{
						CSphString sValue = tReq.GetString();
						tMatch.SetAttr ( tAttr.m_tLocator, (SphAttr_t) sValue.Leak() );
					} else if ( tAttr.m_eAttrType==ESphAttr::SPH_ATTR_FACTORS || tAttr.m_eAttrType==ESphAttr::SPH_ATTR_FACTORS_JSON )
					{
					...
					} else if ( tAttr.m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD )
					{
	...
	ARRAY_FOREACH ( i, dDst )
	{
		...
		if ( iSrcIdx>=0 )
		{
			...
			bool bSame = ( eDst==eSrc )
				|| ( ( eDst==ESphAttr::SPH_ATTR_FLOAT && eSrc==ESphAttr::SPH_ATTR_BOOL ) || ( eDst==ESphAttr::SPH_ATTR_BOOL && eSrc==ESphAttr::SPH_ATTR_FLOAT ) )
				|| ( ( eDst==ESphAttr::SPH_ATTR_BOOL || eDst==ESphAttr::SPH_ATTR_INTEGER || eDst==ESphAttr::SPH_ATTR_BIGINT )
					&& ( eSrc==ESphAttr::SPH_ATTR_BOOL || eSrc==ESphAttr::SPH_ATTR_INTEGER || eSrc==ESphAttr::SPH_ATTR_BIGINT ) );
	...
	if ( iVer>=0x115 )
	{
		...
		ARRAY_FOREACH ( i, tQuery.m_dOverrides )
		{
			...
			ARRAY_FOREACH ( iVal, tOverride.m_dValues )
			{
				...
				if ( tOverride.m_eAttrType==ESphAttr::SPH_ATTR_FLOAT )		tEntry.m_fValue = tReq.GetFloat ();
				else if ( tOverride.m_eAttrType==ESphAttr::SPH_ATTR_BIGINT )	tEntry.m_uValue = tReq.GetUint64 ();
...
int CalcResultLength ( int iVer, const CSphQueryResult * pRes, const CSphTaggedVector & dTag2Pools, bool bAgentMode, const CSphQuery & tQuery, int iMasterVer )
{
	...
	if ( iVer>=0x114 )
	{
		...
		int iWideAttrs = 0;
		for ( int i=0; i<iAttrsCount; i++ )
			if ( pRes->m_tSchema.GetAttr(i).m_eAttrType==ESphAttr::SPH_ATTR_BIGINT )
				iWideAttrs++;
					...
					const CSphTaggedVector & dTag2Pools, bool bAgentMode, const CSphQuery & tQuery, int iMasterVer )
{
	...
	if ( iVer>=0x102 )
	{
		...
		for ( int i=0; i<iAttrsCount; i++ )
		{
			...
			if ( ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_JSON && !bSendJson ) || ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD && !bSendJsonField )
				 || ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_STRINGPTR && !bAgentMode ) )
				eCol = ESphAttr::SPH_ATTR_STRING;
	...
	if ( iVer<=0x101 )
	{
		for ( int i=0; i<pRes->m_tSchema.GetAttrsCount(); i++ )
		{
			...
			if ( iTSLoc.m_iBitOffset<0 && tAttr.m_eAttrType==ESphAttr::SPH_ATTR_TIMESTAMP )
				iTSLoc = tAttr.m_tLocator;

			if ( iGIDLoc.m_iBitOffset<0 && tAttr.m_eAttrType==ESphAttr::SPH_ATTR_INTEGER )
				...
				case ESphAttr::SPH_ATTR_JSON:
					{
						...
						// no break at the end, pass to ESphAttr::SPH_ATTR_STRING
					}
				case ESphAttr::SPH_ATTR_STRING:
					{
						if ( iVer<0x117 )
		...
		for ( int i=iCur; i<iLimit; i++ )
		{
			...
			for ( int j=0; j<pTarget->GetAttrsCount(); j++ )
			{
				...
				} else if ( dMapFrom[j]>=0 )
				{
					...
					if ( tDst.m_eAttrType==ESphAttr::SPH_ATTR_FLOAT && tSrc.m_eAttrType==ESphAttr::SPH_ATTR_BOOL )
					{
...
static void ProcessPostlimit ( const CSphVector<const CSphColumnInfo *> & dPostlimit, int iFrom, int iTo, AggrResult_t & tRes )
{
	...
	for ( int i=iFrom; i<iTo; i++ )
	{
		...
		ARRAY_FOREACH ( j, dPostlimit )
		{
			...
			if ( pCol->m_eAttrType==ESphAttr::SPH_ATTR_INTEGER )
				tMatch.SetAttr ( pCol->m_tLocator, pCol->m_pExpr->IntEval(tMatch) );
			else if ( pCol->m_eAttrType==ESphAttr::SPH_ATTR_BIGINT )
				tMatch.SetAttr ( pCol->m_tLocator, pCol->m_pExpr->Int64Eval(tMatch) );
			else if ( pCol->m_eAttrType==ESphAttr::SPH_ATTR_STRINGPTR )
			{
				const BYTE * pStr = NULL;
	...
	CSphSchema * pExtraSchema, CSphQueryProfile * pProfiler, bool bFromSphinxql, const CSphFilterSettings * pAggrFilter )
{
	...
	ARRAY_FOREACH ( i, dFrontend )
	{
		...
		if ( d.m_iIndex<0 && tItems[i].m_sExpr=="id" )
		{
			...
			d.m_eAttrType = USE_64BIT ? ESphAttr::SPH_ATTR_BIGINT : ESphAttr::SPH_ATTR_INTEGER;
	...
	if ( !p )
	{
		...
		if ( !tQuery.m_sGroupBy.IsEmpty() )
		{
			...
			if ( p && ( p->m_eAttrType!=ESphAttr::SPH_ATTR_STRING && p->m_eAttrType!=ESphAttr::SPH_ATTR_STRINGPTR ) )
				p = NULL;
		}
	}
	if ( p )
		...
		if ( p )
		{
			ARRAY_FOREACH ( i, dFrontend )
			{
				...
				if ( tQuery.m_sGroupBy==d.m_sName &&
					( d.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET
					|| d.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET
					|| d.m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD ) )
				{
					d.m_tLocator = p->m_tLocator;
					d.m_eAttrType = p->m_eAttrType;
					d.m_eAggrFunc = p->m_eAggrFunc;
...
struct ExprHook_t : public ISphExprHook
{
	...
	virtual ESphAttr GetReturnType ( int DEBUGARG(iID), const CSphVector<ESphAttr> & dArgs, bool, CSphString & sError )
	{
		...
		if ( dArgs[0]!=ESphAttr::SPH_ATTR_STRINGPTR && dArgs[0]!=ESphAttr::SPH_ATTR_STRING )
		{
		...
		for ( int i = 1; i < dArgs.GetLength(); i++ )
			if ( dArgs[i]!=ESphAttr::SPH_ATTR_STRING )
			{
				sError.SetSprintf ( "%d argument to SNIPPET() must be a constant string", i );
				return ESphAttr::SPH_ATTR_NONE;
			}

		return ESphAttr::SPH_ATTR_STRINGPTR;
	}
	...
	ARRAY_FOREACH ( iLocal, m_dLocal )
	{
		...
		for ( int i=1; i<dSorters.GetLength() && m_bFacetQueue; i++ )
		{
			...
			for ( int j=0; j<dSorters[i]->GetSchema().GetAttrsCount(); j++ )
				if ( dSorters[i]->GetSchema().GetAttr(j).m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD )
				{
...
public:
	...
	virtual bool Process ( AggrResult_t * pResult, CSphString & sError )
	{
		...
		if ( t!=ESphAttr::SPH_ATTR_INTEGER && t!=ESphAttr::SPH_ATTR_BIGINT && t!=ESphAttr::SPH_ATTR_TOKENCOUNT && t!=ESphAttr::SPH_ATTR_STRINGPTR && t!=ESphAttr::SPH_ATTR_STRING )
		...
		SphAttr_t iLastValue = ( t==ESphAttr::SPH_ATTR_STRING || t==ESphAttr::SPH_ATTR_STRINGPTR )
			? 0
		...
		for ( int i=pResult->m_iOffset; i<Min ( dMatches.GetLength(), pResult->m_iOffset+pResult->m_iCount ); i++ )
		{
			...
			if ( t==ESphAttr::SPH_ATTR_STRING && iCur!=0 )
				iCur = (SphAttr_t)( pResult->m_dTag2Pools [ dMatches[i].m_iTag ].m_pStrings + iCur );

			if ( iCur==iLastValue )
				continue;
			if ( iCur && iLastValue && t==ESphAttr::SPH_ATTR_STRINGPTR && strcmp ( (const char*)iCur, (const char*)iLastValue )==0 )
				continue;
			if ( iCur && iLastValue && t==ESphAttr::SPH_ATTR_STRING )
			{
...
SqlStmt_t::SqlStmt_t ()
	...
	, m_iListStart ( -1 )
	, m_iListEnd ( -1 )
	, m_eUdfType ( ESphAttr::SPH_ATTR_NONE )
	, m_iThreadsCols ( 0 )
	, m_iIntParam ( -1 )
{
...
{
	CSphAttrUpdate & tUpd = m_pStmt->m_tUpdate;
	ESphAttr eType = ESphAttr::SPH_ATTR_UINT32SET;
	...
	if ( dValues.m_pValues.Ptr() && dValues.m_pValues->GetLength()>0 )
	{
		...
		for ( ;pVal<pValMax; pVal++ )
		{
			...
			if ( uVal>UINT_MAX )
			{
				eType = ESphAttr::SPH_ATTR_INT64SET;
			}
...
void UpdateRequestBuilder_t::BuildRequest ( AgentConn_t & tAgent, NetOutputBuffer_c & tOut ) const
{
	...
	{
		assert ( m_tUpd.m_dTypes[i]!=ESphAttr::SPH_ATTR_INT64SET ); // mva64 goes only via SphinxQL (SphinxqlRequestBuilder_t)
		bMva |= ( m_tUpd.m_dTypes[i]==ESphAttr::SPH_ATTR_UINT32SET );
	}
	...
	if ( bMva )
	{
		...
		ARRAY_FOREACH ( iDoc, m_tUpd.m_dDocids )
		{
			...
			{
				if ( m_tUpd.m_dTypes[iAttr]==ESphAttr::SPH_ATTR_UINT32SET )
				{
					DWORD uVal = *pPool++;
					iMvaCount += uVal;
					pPool += uVal;
	...
	ARRAY_FOREACH ( i, m_tUpd.m_dAttrs )
	{
		...
		tOut.SendInt ( ( m_tUpd.m_dTypes[i]==ESphAttr::SPH_ATTR_UINT32SET ) ? 1 : 0 );
	}
...
void HandleCommandUpdate ( ISphOutputBuffer & tOut, int iVer, InputBuffer_c & tReq )
{
	...
	ARRAY_FOREACH ( i, tUpd.m_dAttrs )
	{
		...
		tUpd.m_dTypes[i] = ESphAttr::SPH_ATTR_INTEGER;
		if ( iVer>=0x102 )
		{
			if ( tReq.GetDword() )
			{
				tUpd.m_dTypes[i] = ESphAttr::SPH_ATTR_UINT32SET;
				bMvaUpdate = true;
			}
		}
	}
	...
	for ( int i=0; i<iNumUpdates; i++ )
	{
		...
		{
			if ( tUpd.m_dTypes[iAttr]==ESphAttr::SPH_ATTR_UINT32SET )
			{
	...
	for ( int c=0; c<tStmt.m_iRowsAffected; c++ )
	{
		...
		for ( int i=0; i<iSchemaAttrCount; i++ )
		{
			...
			if ( iQuerySchemaIdx < 0 )
			{
				...
				if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_STRING || tCol.m_eAttrType==ESphAttr::SPH_ATTR_JSON )
					dStrings.Add ( NULL );
			...
			} else
			{
				...
				if ( tVal.m_iType==TOK_CONST_MVA && !( tCol.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET || tCol.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET ) )
				{
					sError.SetSprintf ( "row %d, column %d: MVA value specified for a non-MVA column", 1+c, 1+iQuerySchemaIdx ); // 1 for human base
					break;
				}
				if ( ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET || tCol.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET ) && tVal.m_iType!=TOK_CONST_MVA )
				{
...
void HandleMysqlDescribe ( SqlRowBuffer_c & tOut, SqlStmt_t & tStmt )
{
	...
	{
		const CSphColumnInfo & tCol = tSchema.GetAttr(i);
		if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_INTEGER && tCol.m_tLocator.m_iBitCount!=ROWITEM_BITS )
		{
...
void HandleMysqlUpdate ( SqlRowBuffer_c & tOut, const SqlStmt_t & tStmt, const CSphString & sQuery, CSphString & sWarning, int iCID )
{
	...
	{
		bMvaUpdate = ( tStmt.m_tUpdate.m_dTypes[i]==ESphAttr::SPH_ATTR_UINT32SET
			|| tStmt.m_tUpdate.m_dTypes[i]==ESphAttr::SPH_ATTR_INT64SET );
	}
	...
	for ( int i=0; i<iAttrsCount; i++ )
	{
		...
		} else
		{
			...
			if ( !pExpr || !pExpr->IsConst() )
				eAttrType = ESphAttr::SPH_ATTR_NONE;
			...
			switch ( eAttrType )
			{
				case ESphAttr::SPH_ATTR_STRINGPTR:
					...
					SafeDelete ( pStr );
					break;
				case ESphAttr::SPH_ATTR_INTEGER:	dRows.PutNumeric<int> ( "%d", pExpr->IntEval ( tMatch ) ); break;
				case ESphAttr::SPH_ATTR_BIGINT:	dRows.PutNumeric<SphAttr_t> ( INT64_FMT, pExpr->Int64Eval ( tMatch ) ); break;
				case ESphAttr::SPH_ATTR_FLOAT:	dRows.PutNumeric<float> ( "%f", pExpr->Eval ( tMatch ) ); break;
				default:
					dRows.PutNULL();
					break;
			}
...
void SendMysqlSelectResult ( SqlRowBuffer_c & dRows, const AggrResult_t & tRes, bool bMoreResultsFollow )
{
	...
	} else
	{
		for ( int i=0; i<iSchemaAttrsCount; i++ )
		{
			...
			if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_INTEGER || tCol.m_eAttrType==ESphAttr::SPH_ATTR_TIMESTAMP || tCol.m_eAttrType==ESphAttr::SPH_ATTR_BOOL )
				eType = MYSQL_COL_LONG;
			if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_FLOAT )
				eType = MYSQL_COL_FLOAT;
			if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_BIGINT )
				eType = MYSQL_COL_LONGLONG;
			if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_STRING || tCol.m_eAttrType==ESphAttr::SPH_ATTR_STRINGPTR || tCol.m_eAttrType==ESphAttr::SPH_ATTR_FACTORS || tCol.m_eAttrType==ESphAttr::SPH_ATTR_FACTORS_JSON )
			...
			case ESphAttr::SPH_ATTR_UINT32SET:
				{
					...
					if ( pValues )
					{
						...
						assert ( eAttrType==ESphAttr::SPH_ATTR_UINT32SET || ( nValues%2 )==0 );
						if ( eAttrType==ESphAttr::SPH_ATTR_UINT32SET )
						{
							while ( nValues-- )
							{
			...
			case ESphAttr::SPH_ATTR_JSON:
				{
					...
					if ( eAttrType==ESphAttr::SPH_ATTR_JSON )
					{
			...
			case ESphAttr::SPH_ATTR_FACTORS_JSON:
				{
					...
					{
						dTmp.Resize ( 0 );
						sphFormatFactors ( dTmp, pFactors, eAttrType==ESphAttr::SPH_ATTR_FACTORS_JSON );
						iLen = dTmp.GetLength();
						pStr = dTmp.Begin();
					}
...
void HandleMysqlSelectDual ( SqlRowBuffer_c & tOut, const SqlStmt_t & tStmt )
{
	...
	switch ( eAttrType )
	{
		case ESphAttr::SPH_ATTR_STRINGPTR:
			...
			SafeDelete ( pStr );
			break;
		case ESphAttr::SPH_ATTR_INTEGER:	tOut.PutNumeric<int> ( "%d", pExpr->IntEval ( tMatch ) ); break;
		case ESphAttr::SPH_ATTR_BIGINT:	tOut.PutNumeric<SphAttr_t> ( INT64_FMT, pExpr->Int64Eval ( tMatch ) ); break;
		case ESphAttr::SPH_ATTR_FLOAT:	tOut.PutNumeric<float> ( "%f", pExpr->Eval ( tMatch ) ); break;
		default:
			tOut.PutNULL();
			break;
	}
...
static void AddAttrToIndex ( const SqlStmt_t & tStmt, const ServedIndex_c * pServed, CSphString & sError )
{
	...
	if ( tStmt.m_eAlterColType!=ESphAttr::SPH_ATTR_STRING && pServed->m_pIndex->GetMatchSchema().GetFieldIndex ( sAttrToAdd.cstr () )!=-1 )
	{
...
static void RemoveAttrFromIndex ( const SqlStmt_t & tStmt, const ServedIndex_c * pServed, CSphString & sError )
{
	...
	pServed->m_pIndex->AddRemoveAttribute ( false, sAttrToRemove, ESphAttr::SPH_ATTR_NONE, sError );
}
...
static void HandleMysqlAlter ( SqlRowBuffer_c & tOut, const SqlStmt_t & tStmt, bool bAdd )
{
	...
	if ( bAdd && tStmt.m_eAlterColType==ESphAttr::SPH_ATTR_NONE )
	{
		...
		case STMT_DROP_PLUGIN:
			{
				...
				if ( eStmt==STMT_CREATE_PLUGIN )
					bRes = sphPluginCreate ( pStmt->m_sUdfLib.cstr(), eType, pStmt->m_sUdfName.cstr(), ESphAttr::SPH_ATTR_NONE, m_sError );
				else
	...
	ARRAY_FOREACH ( i, dStmt )
	{
		...
		} else if ( tStmt.m_eStmt==STMT_CREATE_PLUGIN )
		{
			bOk &= sphPluginCreate ( tStmt.m_sUdfLib.cstr(), sphPluginGetType ( tStmt.m_sStringParam ),
				tStmt.m_sUdfName.cstr(), ESphAttr::SPH_ATTR_NONE, *sError );
		} else
		{
			bOk = false;
