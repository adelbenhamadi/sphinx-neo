...
static ESphOnFileFieldError	g_eOnFileFieldError = ESphOnFileFieldError::FFE_IGNORE_FIELD;

#if USE_WINDOWS
static char			g_sMinidump[256];
#endif

// rotate interval 100 ms;
constexpr auto ROTATE_MIN_INTERVAL = 100000;
...
#define LOC_TEXT()			{ if ( *sBuf!=';') LOC_ERR ( "';'", sBuf ); sTok = ++sBuf; while ( *sBuf && *sBuf!=';' ) sBuf++; iTokLen = sBuf-sTok; }
		...
		// handle ATTR-TYPE
		LOC_SPACE0(); LOC_TOK();
		if (LOC_TOKEQ("uint"))				tAttr.m_eAttrType = ESphAttr::SPH_ATTR_UINT32SET;
		else if (LOC_TOKEQ("timestamp"))		tAttr.m_eAttrType = ESphAttr::SPH_ATTR_UINT32SET;
		else if (LOC_TOKEQ("bigint"))			tAttr.m_eAttrType = ESphAttr::SPH_ATTR_INT64SET;
		...
		if (LOC_TOKEQ("field"))				tAttr.m_eSrc = ESphAttrSrc::SPH_ATTRSRC_FIELD;
		else if (LOC_TOKEQ("query"))			tAttr.m_eSrc = ESphAttrSrc::SPH_ATTRSRC_QUERY;
		else if (LOC_TOKEQ("ranged-query"))	tAttr.m_eSrc = ESphAttrSrc::SPH_ATTRSRC_RANGEDQUERY;
		else									LOC_ERR("value source type ('field', or 'query', or 'ranged-query')", sTok);

		if (tAttr.m_eSrc == ESphAttrSrc::SPH_ATTRSRC_FIELD)	return true;
		...
		if (tAttr.m_eSrc == ESphAttrSrc::SPH_ATTRSRC_QUERY)	return true;
...
#undef LOC_TEXT
	...
	bool SqlParamsConfigure(CSphSourceParams_SQL& tParams, const CSphConfigSection& hSource, const char* sSourceName)
	{
		...
		SqlAttrsConfigure(tParams, hSource("sql_attr_uint"), ESphAttr::SPH_ATTR_INTEGER, sSourceName);
		SqlAttrsConfigure(tParams, hSource("sql_attr_timestamp"), ESphAttr::SPH_ATTR_TIMESTAMP, sSourceName);
		SqlAttrsConfigure(tParams, hSource("sql_attr_bool"), ESphAttr::SPH_ATTR_BOOL, sSourceName);
		SqlAttrsConfigure(tParams, hSource("sql_attr_float"), ESphAttr::SPH_ATTR_FLOAT, sSourceName);
		SqlAttrsConfigure(tParams, hSource("sql_attr_bigint"), ESphAttr::SPH_ATTR_BIGINT, sSourceName);
		SqlAttrsConfigure(tParams, hSource("sql_attr_string"), ESphAttr::SPH_ATTR_STRING, sSourceName);
		SqlAttrsConfigure(tParams, hSource("sql_attr_json"), ESphAttr::SPH_ATTR_JSON, sSourceName);

		SqlAttrsConfigure(tParams, hSource("sql_field_string"), ESphAttr::SPH_ATTR_STRING, sSourceName, true);
...
#else
		fprintf(stdout, "WARNING: source '%s': TsvPipe support NOT compiled in. To use TsvPipe, "
			"reconfigure, and rebuild Sphinx\n", sSourceName);
		return NULL;
#endif // USE_SV
	}
...
#else
		fprintf(stdout, "WARNING: source '%s': CsvPipe support NOT compiled in. To use CsvPipe, "
			"reconfigure, and rebuild Sphinx\n", sSourceName);
		return NULL;
#endif // USE_SV
	}
...
LONG WINAPI sigsegv(EXCEPTION_POINTERS* pExc)
{
	...
	::_write(STDERR_FILENO, sFail1,(unsigned int) strlen(sFail1));
	::_write(STDERR_FILENO, g_sMinidump, (unsigned int)strlen(g_sMinidump));
	::_write(STDERR_FILENO, sFail2, (unsigned int)strlen(sFail2));
	::_write(STDERR_FILENO, sFailVer, (unsigned int)strlen(sFailVer));
...
int main(int argc, char** argv)
{
	...
	STATS::sphInitIOStats();
	STATS::CSphIOStats tIO;
	tIO.Start();
