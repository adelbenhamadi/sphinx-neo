	...
	CSphString & sError, CSphString & sWarning, ISphRtAccum * pAccExt )
{
	...
	if ( pAcc )
	{
		...
		for ( int i=0; i<tSchema.GetAttrsCount(); i++ )
		{
			...
			if ( tColumn.m_eAttrType==ESphAttr::SPH_ATTR_JSON )
			{
			...
			iAttr += ( tColumn.m_eAttrType==ESphAttr::SPH_ATTR_STRING || tColumn.m_eAttrType==ESphAttr::SPH_ATTR_JSON ) ? 1 : 0;
		}
...
void RtAccum_t::AddDocument ( ISphHits * pHits, const CSphMatch & tDoc, bool bReplace, int iRowSize, const char ** ppStr, const CSphVector<DWORD> & dMvas, const CSphVector<JSONAttr_t> & dJson )
{
	...
	for ( int i=0; i<tSchema.GetAttrsCount(); i++ )
	{
		...
		const CSphColumnInfo & tColumn = tSchema.GetAttr(i);
		if ( tColumn.m_eAttrType==ESphAttr::SPH_ATTR_STRING || tColumn.m_eAttrType==ESphAttr::SPH_ATTR_JSON )
		{
			const char * pStr = ppStr ? ppStr[iAttr++] : NULL;
			int iLen = pStr ? strlen ( pStr ) : 0;

			CSphVector<BYTE> dBuf;
			if ( pStr && iLen && tColumn.m_eAttrType==ESphAttr::SPH_ATTR_JSON )
			{
		...
		} else if ( tColumn.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET || tColumn.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET )
		{
	...
	if ( m_pIndex->GetSettings().m_bIndexFieldLens )
	{
		...
		assert ( tSchema.GetAttr ( iFirst ).m_eAttrType==ESphAttr::SPH_ATTR_TOKENCOUNT );
		assert ( tSchema.GetAttr ( iFirst+tSchema.m_dFields.GetLength()-1 ).m_eAttrType==ESphAttr::SPH_ATTR_TOKENCOUNT );
...
void VerifyEmptyStrings ( const CSphTightVector<BYTE> & dStorage, const CSphSchema & tSchema, const CSphRowitem * pRow )
{
	...
	for ( int i=0; i<tSchema.GetAttrsCount(); i++ )
	{
		...
		assert ( tCol.m_eAttrType!=ESphAttr::SPH_ATTR_STRING
		|| ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_STRING && sphGetRowAttr ( pAttr, tCol.m_tLocator )==0 ) );
	}
}
#endif
...
public:
	explicit StorageStringWriter_t ( const CSphSchema & tSchema, CSphWriter & tDst )
		: m_tDst ( tDst )
	{
		ExtractLocators ( tSchema, ESphAttr::SPH_ATTR_STRING, m_dLocators );
		ExtractLocators ( tSchema, ESphAttr::SPH_ATTR_JSON, m_dLocators );
	}
...
public:
	explicit StorageMvaWriter_t ( const CSphSchema & tSchema, CSphWriter & tDst )
		: m_tDst ( tDst )
	{
		ExtractLocators ( tSchema, ESphAttr::SPH_ATTR_UINT32SET, m_dLocators );
		ExtractLocators ( tSchema, ESphAttr::SPH_ATTR_INT64SET, m_dLocators );
	}
	const CSphVector<CSphAttrLocator> & GetLocators () const { return m_dLocators; }
...
#undef LOC_ESTIMATE1
	...
	// lengths might be empty on delete
	if ( m_tSchema.GetAttrId_FirstFieldLen()>=0 && dLens.GetLength()>0 )
...
int RtIndex_t::DebugCheck ( FILE * fp )
{
	...
	ARRAY_FOREACH ( iSegment, m_dRamChunks )
	{
		...
		for ( int iAttr=0; iAttr<m_tSchema.GetAttrsCount(); iAttr++ )
		{
			...
			else if ( tAttr.m_eAttrType==ESphAttr::SPH_ATTR_STRING )
				dStrItems.Add ( tAttr.m_tLocator );
			else if ( tAttr.m_eAttrType==ESphAttr::SPH_ATTR_JSON )
...
struct CSphAttrTypedLocator : public CSphAttrLocator
{
	...
	CSphAttrTypedLocator()
		: m_eAttrType ( ESphAttr::SPH_ATTR_NONE )
	{}
	...
	virtual void Process ( CSphMatch * pMatch )
	{
		...
		ARRAY_FOREACH ( i, m_dGetLoc )
		{
			...
			case ESphAttr::SPH_ATTR_JSON:
			{
				...
				if ( tLoc.m_eAttrType==ESphAttr::SPH_ATTR_JSON && m_dJsonAssoc.GetLength() )
				{
	...
	ISphMatchSorter ** ppSorters, const CSphMultiQueryArgs & tArgs ) const
{
	...
	for ( int i=0; i<dSorters[iMaxSchemaIndex]->GetSchema().GetAttrsCount(); i++ )
	{
		...
		if ( tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_STRING || tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_JSON
			|| tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET || tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET )
		{
			const CSphColumnInfo * pCol = m_tSchema.GetAttr ( tSetInfo.m_sName.cstr() );
			if ( !pCol && ( tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET || tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET ) )
			{
				pCol = &tSetInfo;
			}
			assert ( pCol );
		...
		iJsonFields += ( tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD );
	}
	...
	if ( iJsonFields )
	{
		...
		{
			const CSphColumnInfo & tSetInfo = dSorters[iMaxSchemaIndex]->GetSchema().GetAttr(i);
			if ( tSetInfo.m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD )
			{
				const int iInLocator = dSorters[iMaxSchemaIndex]->GetSchema().GetAttrIndex ( tSetInfo.m_sName.cstr() );
				assert ( iInLocator>=0 );

				dGetLoc.Add().Set ( dSorters[iMaxSchemaIndex]->GetSchema().GetAttr ( iInLocator ).m_tLocator, ESphAttr::SPH_ATTR_JSON_FIELD );
				dSetLoc.Add ( tSetInfo.m_tLocator );
			}
		}
	}
...
int RtIndex_t::UpdateAttributes ( const CSphAttrUpdate & tUpd, int iIndex, CSphString & sError, CSphString & sWarning )
{
	...
	ARRAY_FOREACH ( i, tUpd.m_dAttrs )
	{
		...
		if ( iIdx>=0 )
		{
			...
			bHasMva |= ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_UINT32SET || tCol.m_eAttrType==ESphAttr::SPH_ATTR_INT64SET );
	...
	for ( int iUpd=iFirst; iUpd<iLast; iUpd++ )
	{
		...
		for ( ;; )
		{
			...
			ARRAY_FOREACH ( iCol, tUpd.m_dAttrs )
			{
				...
				if ( !( tUpd.m_dTypes[iCol]==ESphAttr::SPH_ATTR_UINT32SET || tUpd.m_dTypes[iCol]==ESphAttr::SPH_ATTR_INT64SET ) )
				{
					// plain update
					bUpdated = true;
...
void RtBinlog_c::BinlogUpdateAttributes ( int64_t * pTID, const char * sIndexName, const CSphAttrUpdate & tUpd )
{
	...
	ARRAY_FOREACH ( i, tUpd.m_dAttrs )
	{
		...
		m_tWriter.ZipOffset ((size_t) tUpd.m_dTypes[i] );
	}
...
bool sphRTSchemaConfigure ( const CSphConfigSection & hIndex, CSphSchema * pSchema, CSphString * pError )
{
	...
	const ESphAttr iTypes[iNumTypes] = { ESphAttr::SPH_ATTR_INTEGER, ESphAttr::SPH_ATTR_BIGINT, ESphAttr::SPH_ATTR_FLOAT, ESphAttr::SPH_ATTR_TIMESTAMP, ESphAttr::SPH_ATTR_STRING, ESphAttr::SPH_ATTR_UINT32SET, ESphAttr::SPH_ATTR_INT64SET, ESphAttr::SPH_ATTR_JSON, ESphAttr::SPH_ATTR_BOOL };
	...
	for ( int iType=0; iType<iNumTypes; iType++ )
	{
		for ( CSphVariant * v = hIndex ( sTypes[iType] ); v; v = v->m_pNext )
		{
			...
			if ( pColon )
			{
				if ( tCol.m_eAttrType==ESphAttr::SPH_ATTR_INTEGER )
				{
				...
				tCol.m_sName.SwapWith ( sName );
			}

			pSchema->AddAttr ( tCol, false );

			if ( tCol.m_eAttrType!=ESphAttr::SPH_ATTR_STRING && hFields.Exists ( tCol.m_sName ) )
			{
