...
#define offsetof(T, M) \
	...
	PluginDesc_c* sphPluginAcquire(const char* szLib, PluginType_e eType, const char* szName, CSphString& sError)
	{
		...
		if (!pDesc)
		{
			if (!sphPluginCreate(szLib, eType, szName, ESphAttr::SPH_ATTR_NONE, sError))
				return NULL;
	...
	static const char* UdfReturnType(ESphAttr eType)
	{
		switch (eType)
		{
		case ESphAttr::SPH_ATTR_INTEGER:		return "INT";
		case ESphAttr::SPH_ATTR_FLOAT:		return "FLOAT";
		case ESphAttr::SPH_ATTR_STRINGPTR:	return "STRING";
