...
public:
	FilterString_c ( ESphCollation eCollation, ESphAttr eType, bool bEq )
		...
		, m_pStringBase ( NULL )
		, m_bPacked ( eType==ESphAttr::SPH_ATTR_STRING )
	{}
	...
	const CSphAttrLocator & tLoc, CSphString & sError, CSphString & sWarning, bool bHasEqual, ESphCollation eCollation )
{
	...
	if ( eAttrType==ESphAttr::SPH_ATTR_UINT32SET || eAttrType==ESphAttr::SPH_ATTR_INT64SET )
	{
		...
		bool bWide = ( eAttrType==ESphAttr::SPH_ATTR_INT64SET );
	...
	// float
	if ( eAttrType==ESphAttr::SPH_ATTR_FLOAT )
	{
	...
	if ( eAttrType==ESphAttr::SPH_ATTR_STRING || eAttrType==ESphAttr::SPH_ATTR_STRINGPTR )
	{
	...
	switch ( eFilterType )
	{
		case SPH_FILTER_VALUES:
			if ( iNumValues==1 && ( eAttrType==ESphAttr::SPH_ATTR_INTEGER || eAttrType==ESphAttr::SPH_ATTR_BIGINT || eAttrType==ESphAttr::SPH_ATTR_TOKENCOUNT ) )
			{
				if ( ( eAttrType==ESphAttr::SPH_ATTR_INTEGER || eAttrType==ESphAttr::SPH_ATTR_TOKENCOUNT ) && !tLoc.m_bDynamic && tLoc.m_iBitCount==32 && ( tLoc.m_iBitOffset % 32 )==0 )
...
static ISphFilter * CreateFilterExpr ( ISphExpr * pExpr, ESphFilter eType, bool bRangeEq, CSphString & sError, ESphCollation eCollation, ESphAttr eAttrType )
{
	...
	if ( eAttrType==ESphAttr::SPH_ATTR_JSON || eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD )
	...
	CSphString & sError, CSphString & sWarning, bool bHaving, ESphCollation eCollation, bool bArenaProhibit )
{
	...
	if ( !pFilter )
	{
		...
		} else
		{
			...
			if ( pAttr->m_eAttrType==ESphAttr::SPH_ATTR_JSON || pAttr->m_eAttrType==ESphAttr::SPH_ATTR_JSON_FIELD )
			{
			...
			} else
			{
				...
				if ( pAttr->m_eAttrType==ESphAttr::SPH_ATTR_FLOAT && tSettings.m_eType==SPH_FILTER_VALUES && tSettings.GetNumValues()==1 )
				{
