...
/// morpohological form info
	...
	public:
		...
		BYTE* GetToken()
		{
			m_eTokenMorph = ESphTokenMorph::SPH_TOKEN_MORPH_RAW;
			...
			if (m_iCurrent >= 0)
			{
				...
				if (m_FindResults[m_iCurrent] == AOT_ORIGFORM)
				{
					...
					m_iCurrent = -1;
					m_eTokenMorph = ESphTokenMorph::SPH_TOKEN_MORPH_ORIGINAL;
					return m_sToken;
				}
				...
				m_eTokenMorph = ESphTokenMorph::SPH_TOKEN_MORPH_GUESS;
				return m_sToken;
			}
	...
	void sphAotShutdown()
	{
		for (int i = 0; i < AOT_LANGS::AOT_LENGTH; i++)
